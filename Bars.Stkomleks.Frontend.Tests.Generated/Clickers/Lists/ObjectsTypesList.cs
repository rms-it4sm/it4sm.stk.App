...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Типы объектов
    /// </summary>       
    [Display("Кликер списка [Типы объектов]")]
    [Display("Список [Типы объектов]")]
    public class ObjectsTypesListClicker : WebElementClicker<ObjectsTypesListPageObject>
    {          
        public ObjectsTypesListClicker(AbstractWebDriverTest test, ObjectsTypesListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Типы объектов]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Тип объекта]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<ObjectsTypesEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

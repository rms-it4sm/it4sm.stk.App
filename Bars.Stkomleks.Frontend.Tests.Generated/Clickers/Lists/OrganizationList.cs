...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Организации
    /// </summary>       
    [Display("Кликер списка [Организации]")]
    [Display("Список [Организации]")]
    public class OrganizationListClicker : WebElementClicker<OrganizationListPageObject>
    {          
        public OrganizationListClicker(AbstractWebDriverTest test, OrganizationListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Организации]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Организация]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<OrganizationEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Контакты.Организация
    /// </summary>       
    [Display("Кликер списка [Контакты.Организация]")]
    [Display("Список [Контакты.Организация]")]
    public class OrganizationContactsListClicker : WebElementClicker<OrganizationContactsListPageObject>
    {          
        public OrganizationContactsListClicker(AbstractWebDriverTest test, OrganizationContactsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Контакты.Организация]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Контакт организации]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<OrganizationContactsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

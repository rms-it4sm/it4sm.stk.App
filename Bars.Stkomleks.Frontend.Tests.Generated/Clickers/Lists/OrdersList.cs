...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Заказы
    /// </summary>       
    [Display("Кликер списка [Заказы]")]
    [Display("Список [Заказы]")]
    public class OrdersListClicker : WebElementClicker<OrdersListPageObject>
    {          
        public OrdersListClicker(AbstractWebDriverTest test, OrdersListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Заказы]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Заказ]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<OrdersEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

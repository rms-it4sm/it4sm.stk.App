...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Занимаемые должности.Подразделение
    /// </summary>       
    [Display("Кликер списка [Занимаемые должности.Подразделение]")]
    [Display("Список [Занимаемые должности.Подразделение]")]
    public class CurrentPositionsDepartmentsListClicker : WebElementClicker<CurrentPositionsDepartmentsListPageObject>
    {          
        public CurrentPositionsDepartmentsListClicker(AbstractWebDriverTest test, CurrentPositionsDepartmentsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Занимаемые должности.Подразделение]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Занимаемая должность подразделения]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<CurrentPositionsDepartmentsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Сотрудники.Организация
    /// </summary>       
    [Display("Кликер списка [Сотрудники.Организация]")]
    [Display("Список [Сотрудники.Организация]")]
    public class StaffListClicker : WebElementClicker<StaffListPageObject>
    {          
        public StaffListClicker(AbstractWebDriverTest test, StaffListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Сотрудники.Организация]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Сотрудник]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<StaffEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

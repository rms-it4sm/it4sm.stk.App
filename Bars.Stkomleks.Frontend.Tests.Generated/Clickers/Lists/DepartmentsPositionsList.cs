...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Должности.Подразделение
    /// </summary>       
    [Display("Кликер списка [Должности.Подразделение]")]
    [Display("Список [Должности.Подразделение]")]
    public class DepartmentsPositionsListClicker : WebElementClicker<DepartmentsPositionsListPageObject>
    {          
        public DepartmentsPositionsListClicker(AbstractWebDriverTest test, DepartmentsPositionsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Должности.Подразделение]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Должность подразделения]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<DepartmentsPositionsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

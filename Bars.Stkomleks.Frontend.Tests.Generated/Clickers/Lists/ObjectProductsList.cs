...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Спецификация.Объект
    /// </summary>       
    [Display("Кликер списка [Спецификация.Объект]")]
    [Display("Список [Спецификация.Объект]")]
    public class ObjectProductsListClicker : WebElementClicker<ObjectProductsListPageObject>
    {          
        public ObjectProductsListClicker(AbstractWebDriverTest test, ObjectProductsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Спецификация.Объект]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Элемент объекта]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<ObjectProductsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

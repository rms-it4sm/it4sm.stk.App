...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Специалисты.Объект
    /// </summary>       
    [Display("Кликер списка [Специалисты.Объект]")]
    [Display("Список [Специалисты.Объект]")]
    public class CompositionObjectSpecialistsListClicker : WebElementClicker<CompositionObjectSpecialistsListPageObject>
    {          
        public CompositionObjectSpecialistsListClicker(AbstractWebDriverTest test, CompositionObjectSpecialistsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Специалисты.Объект]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Специалист объекта]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<CompositionObjectSpecialistsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

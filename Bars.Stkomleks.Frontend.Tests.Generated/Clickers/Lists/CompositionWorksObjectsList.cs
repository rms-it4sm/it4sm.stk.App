...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Работы.Объект
    /// </summary>       
    [Display("Кликер списка [Работы.Объект]")]
    [Display("Список [Работы.Объект]")]
    public class CompositionWorksObjectsListClicker : WebElementClicker<CompositionWorksObjectsListPageObject>
    {          
        public CompositionWorksObjectsListClicker(AbstractWebDriverTest test, CompositionWorksObjectsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Работы.Объект]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Работа объекта]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<CompositionWorksObjectsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

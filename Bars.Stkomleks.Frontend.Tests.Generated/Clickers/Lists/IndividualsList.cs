...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Физические лица
    /// </summary>       
    [Display("Кликер списка [Физические лица]")]
    [Display("Список [Физические лица]")]
    public class IndividualsListClicker : WebElementClicker<IndividualsListPageObject>
    {          
        public IndividualsListClicker(AbstractWebDriverTest test, IndividualsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Физические лица]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Физическое лицо]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<IndividualsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

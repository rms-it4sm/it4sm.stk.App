...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Организации.Заказ
    /// </summary>       
    [Display("Кликер списка [Организации.Заказ]")]
    [Display("Список [Организации.Заказ]")]
    public class OrderOrganizationListClicker : WebElementClicker<OrderOrganizationListPageObject>
    {          
        public OrderOrganizationListClicker(AbstractWebDriverTest test, OrderOrganizationListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Организации.Заказ]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Организация заказа]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<OrderOrganizationEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

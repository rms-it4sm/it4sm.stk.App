...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Документы.Объект
    /// </summary>       
    [Display("Кликер списка [Документы.Объект]")]
    [Display("Список [Документы.Объект]")]
    public class ObjectsDocumentsListClicker : WebElementClicker<ObjectsDocumentsListPageObject>
    {          
        public ObjectsDocumentsListClicker(AbstractWebDriverTest test, ObjectsDocumentsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Документы.Объект]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Документ объекта]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<ObjectsDocumentsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

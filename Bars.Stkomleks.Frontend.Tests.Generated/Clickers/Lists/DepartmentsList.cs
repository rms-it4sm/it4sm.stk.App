...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Подразделения.Организация
    /// </summary>       
    [Display("Кликер списка [Подразделения.Организация]")]
    [Display("Список [Подразделения.Организация]")]
    public class DepartmentsListClicker : WebElementClicker<DepartmentsListPageObject>
    {          
        public DepartmentsListClicker(AbstractWebDriverTest test, DepartmentsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Подразделения.Организация]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Подразделение]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<DepartmentsEditorClicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

...
using Bars.Stkomleks.Frontend;
    ...
    /// <summary>    
    /// Класс-кликер, содержащий код для обхода списка Объекты.Заказ
    /// </summary>       
    [Display("Кликер списка [Объекты.Заказ]")]
    [Display("Список [Объекты.Заказ]")]
    public class OrdersObjectsListClicker : WebElementClicker<OrdersObjectsListPageObject>
    {          
        public OrdersObjectsListClicker(AbstractWebDriverTest test, OrdersObjectsListPageObject view) : base(test, view) {}

        public override void Run()
        {
            Group("Проверяем представление [Объекты.Заказ]", 
    () => 
    {
        ...
        Step("Обходим записи и открываем каждую на редактирование", 
            () => {
                ...
                Step("Открытие записи редактором [Объект.Изменение]", 
                    () => {
                        ...
                        for (var index = 0; index < records.Length; index++) {
                            Step($"Открытие редактора для записи #{index}", 
                                () => {
                                    ...
                                    {
                                        Test.RunClicker<OrdersObjectsEditor_Copy_0Clicker>(editor);
                                        editor.Close();
                                    }
                                }
                            );

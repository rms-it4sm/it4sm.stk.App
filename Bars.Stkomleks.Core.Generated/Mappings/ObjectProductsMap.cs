namespace Bars.Stkomleks.Mappings
{
    ...
    /// <summary>
    /// Мапинг сущности Спецификация объекта
    /// </summary>
    public class ObjectProductsMap : ClassMapping<Bars.Stkomleks.ObjectProducts>
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        public ObjectProductsMap()         {
			Polymorphism(PolymorphismType.Explicit);
            Table("OBJECTPRODUCTS");
...
#region Class: ObjectProducts
ManyToOne(x => x.ObjectOrder, m => { m.Column("OBJECTORDER");m.Cascade(Cascade.Persist);});
Property(x => x.Code, p => { p.Column(col => { col.Name("CODE");});});
ManyToOne(x => x.TypeElement, m => { m.Column("TYPEELEMENT");m.Cascade(Cascade.Persist);});
Property(x => x.Quantity, p => { p.Column(col => { col.Name("QUANTITY");});});
ManyToOne(x => x.UnitMeasure, m => { m.Column("UNITMEASURE");m.Cascade(Cascade.Persist);});
